...
def handle_archive(fullname: Path, target_folder: Path) -> None:
    ...
    """
    target_folder.mkdir(exist_ok=True, parents=True)
    name_without_extension = fullname.name[:fullname.name.rfind(scan.get_extension(fullname.name))-1]
    folder_for_file = target_folder.joinpath(name_without_extension)
    folder_for_file = target_folder.joinpath(check_new_name(target_folder, folder_for_file))
